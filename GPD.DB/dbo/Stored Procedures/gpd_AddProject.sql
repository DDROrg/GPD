	...
	BEGIN TRY
		BEGIN TRAN;
			...
			SELECT
				...
				M.value('(application/plugin-name)[1]', 'NVARCHAR(150)'),
			...
			--- ROSS
			IF @P_UserId > 0 AND EXISTS (SELECT * FROM gpd_user_details u WHERE u.user_id = @P_UserId)
				BEGIN
					INSERT INTO [gpd_project_user_xref] 
					VALUES(@P_ProjectId, @P_UserId, getdate(), NULL);
				END;
